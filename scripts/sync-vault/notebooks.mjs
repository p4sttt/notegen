...
function makeCodeFence(source) {
  const longestFence =
    source.match(/`{3,}/g)?.reduce((max, fence) => Math.max(max, fence.length), 0) ?? 0;
...
function pickNotebookNoteMetadata(notebook) {
  ...
  const scopedNotegenMetadata = isRecord(notegenMetadata.note)
    ? notegenMetadata.note
    : notegenMetadata;
  const rootMetadata = {};
...
export function createNotebookConverter({ assetsRoot, publicBasePath }) {
  ...
  function renderNotebookOutput(output, publicScope, cellIndex, outputIndex) {
    ...
    if (outputType === 'stream') {
      ...
      return text
        ? `<pre class="notebook-output notebook-output-stream"><code>${escapeHtml(text)}</code></pre>`
        : '';
    }
    ...
    const imageMime = ['image/png', 'image/jpeg', 'image/webp', 'image/svg+xml'].find(
      (mimeType) => data[mimeType],
    );
    if (imageMime) {
      const imageUrl = writeNotebookImageOutput(
        data[imageMime],
        imageMime,
        publicScope,
        cellIndex,
